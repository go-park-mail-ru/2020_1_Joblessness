...
func RegisterHTTPEndpoints(router *mux.Router, m *middleware.AuthMiddleware, uc summaryInterfaces.SummaryUseCase) {
	...
	router.HandleFunc("/summaries/{vacancy_id}/response", m.CheckAuth(h.SendSummary)).Methods("POST")
	router.HandleFunc("/summaries/{summary_id}/response", m.CheckAuth(h.ResponseSummary)).Methods("PUT")
